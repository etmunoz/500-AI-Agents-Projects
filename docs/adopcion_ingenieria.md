...
## Integración a `main` · 2026-09-11

Los pasos 0 a 6 se trabajaron en la rama `adopcion-ingenieria` y se integraron a `main` con un merge sin *fast-forward*, para que la adopción quede legible como un bloque coherente y no disuelta entre commits sueltos.

**10 commits, 67 archivos, +4797 / −128.**

| Commit | Qué trajo |
|---|---|
| `9cdc16c` | `AGENTS.md`, `CLAUDE.md` y el diagnóstico (pasos 0 y 1) |
| `cb66d4c` | `.gitignore`, `.gitattributes` y los hooks (paso 2) |
| `52731ed` | El bit de ejecución de los hooks, que en Windows no se había registrado |
| `9d22426` | `docs/reglas/reglas_desarrollo.md` (paso 3) |
| `f1edb63` | `docs/roadmap.md` y la corrección del conteo de referencias al upstream (paso 4) |
| `d924ba4` | `README.md` al estándar, 7 de 13 referencias corregidas (paso 5) |
| `4cb1e4d` | Los dos manuales, `SECURITY.md` partido, contrato de README de agente (paso 6) |
| `f3df1b2` | `uv`, y la reparación del escáner de secretos que se caía |
| `53fb522` | Migración de los 21 agentes a `uv`; el agente de referencia, reparado |
| `0d70439` | Guarda de codificación en 23 archivos; lockfile del catálogo y `npm ci` |

**Estado al integrar**, verificado ejecutando y no leyendo:

| Comprobación | Resultado |
|---|---|
| `pre-push` → markdown | ok — 26 archivos, 0 hallazgos |
| `pre-push` → agentes (sintaxis) | ok — 25 de 25 compilan |
| `pre-push` → web (build) | ok |
| `pre-commit` | ok — los dos motores, `gitleaks` y `revisor-local` |
| Agente 01, de punta a punta | instala y corre hasta el 401 de la clave de ejemplo |

**Lo que la integración NO resuelve, y sigue abierto:** cero pruebas, los 20 agentes cuya instalación no se probó una por una, las 6 referencias al upstream que quedan, y los pasos 7 a 9.
