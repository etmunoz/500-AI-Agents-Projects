...
**2. Este fork diverge del upstream.** Decisión del PM del 2026-09-11. El repositorio nació como fork de `ashishpatel26/500-AI-Agents-Projects` y quedan nueve referencias al original —badges, el `git clone` del Quick Start, el enlace de issues, el gráfico de estrellas, el contacto de seguridad y los `assignees` de las plantillas de issue—. Se reescriben a `etmunoz/` en los pasos 5 y 8. **No agregues referencias nuevas al upstream**, y si tocás alguna de las que quedan, corregila de paso.
